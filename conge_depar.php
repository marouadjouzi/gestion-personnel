...
if (!empty($resultat444)) {
    foreach ($resultat444 as $line444) {
        ...
        $requete = "SELECT * FROM responsabl WHERE approbation = :departement";
        $stat = $conn->prepare($requete);


        
        $stat->bindParam(':departement', $_SESSION['approbation'], PDO::PARAM_STR);
        $stat->execute();
        $result = $stat->fetchAll(PDO::FETCH_ASSOC);
        foreach ($result as $line) {
        $id_receive2 = $line['id'];
        echo '<script>console.log('.$id_receive2.')</script>';

    $sql = "SELECT * FROM notifications WHERE id_receive = ? OR id_receive = ?  ORDER BY is_read ASC";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$id_receive, $id_receive2]);
                    ...
                    <div class="nav__list">
                       ...
                       <?php if($_SESSION['role'] == 'superadmin' || $_SESSION['role'] == 'admin'){?>
                        ...
                        <?php }else {
                            ...
                            if ($id_departement) {
                                ?>
                                <a href="conge_depar.php" class="nav__link active">
                                    <ion-icon name="calendar-outline" class="nav__icon"></ion-icon>
                                    <span class="nav__name">Congés</span>
                                </a>
                                <?php
                        ...
                        <ion-icon name="stats-chart-outline" style="font-size:25px;"></ion-icon>
                            <span class="nav__name">Suivi Informations</span>
                        </a>
                    </div>
                </div>
                ...
                while ($row444 = mysqli_fetch_assoc($result444)) {
                    if ($row444['nom_departement'] == $_SESSION['approbation']) {
                        if ($row4['type_conge'] == 'annuel detente' && $row4['val_departement'] == '0') {
                            ...
                            <table class="table-container">
                                ...
                                <tbody style="background-color: white;">
                                    <tr>
                                        ...
                                        <?php if ($row4['val_departement'] == "0") {
                                                ?>
                                                <form action="conge_depar.php" method="post"><td>
                            ...
                            ?>
                            <h2> Demande de congés de recuperation</h2>
                            <table class="table-container">
                                <thead>
                                    <tr>
                                        <th>ID_Employe</th>
                            ...
                            <?php
                        } else if ($row4['type_conge'] == 'exceptionel' && $row4['val_departement'] == '0') {
                            ?>
                            <h2> Demande de congé exceptionnel</h2>
                            <table class="table-container">
                                <thead>
                                    <tr>
                                        <th>ID_Employe</th>
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['id'])) {
        ...
        if ($selectedOption == 'oui') {
            if (isset($_POST['emp']) && isset($_POST['type'])) {
                $emp = $_POST['emp'];
                $type = $_POST['type'];
                ...
                if ($employee44->rowCount() > 0) {
                    ...
                    if ($stmt444->rowCount() > 0) {
                       while ($line444 = $stmt444->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $id_receive = $emp;
                        $id_send = $line444['id_departement'];
                        $message = "Votre demande de congé " . $type . " est validée au niveau de votre département";
                        ...
                        $requete4444 = "SELECT * FROM direction WHERE id_direction = :id";
                        $stmt4444 = $db->prepare($requete4444);
                        $stmt4444->bindParam(':id', $line444['d_direction']);
                        $stmt4444->execute();
                        ...
                        if ($stmt4444->rowCount() > 0) {
                            ...
                            $id_send = $line444['id_departement'];
                            $message2 = "Vous avez reçu une demande de congé " . $type . " de " . $line44['nom'] . " " . $line44['prenom'];
                            $id_receive2 = $line4444['id_direction'];
            ...
            if ($employee44->rowCount() > 0) {
                ...
                if ($stmt444->rowCount() > 0) {
                   while ($line444 = $stmt444->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $id_receive = $emp;
                    $id_send = $line444['id_departement'];
                    $message = "Votre demande de congé " . $type . " est refusée au niveau de votre département";
...
<div id="table2" >
<p id="title1">Etat des demandes  :</p>
...
?>
                    <tr>
                        ...
                        <?php if ($line4['val_departement'] == '1' && $line4['val_direction'] == '0') { ?>
                            <td>Direction</td>
                            <td>En attente</td>
                            <td style="background-color:#e6e8ed; border:none">
                             <form method = "POST" action="conge_depar.php">
                        ...
                        <?php } else if ($line4['val_departement'] == '2') { ?>
                            <td>Departement</td>
                            <td>Refuse</td>
                            <td style="background-color:#e6e8ed; border:none"><form method = "POST" action="conge_depar.php">
                                <input type="hidden" name="id" value="<?php echo $line4['id_demande']; ?>"/>
                                <button type="submit" name="modifier" style="background-color:rgb(84, 169, 90); color:#fff; border-radius:3px; font-size:xx-large">
                                    <i class='bx bxs-edit'></i> 
                                </button>
                        ...
                        <?php } else if (  $line4['val2_rh'] == '1') { ?> 
                            <td>Ressource Humaines</td>
                            <td>En attente de signature de titre de conge</td>   
                        <?php } ?>   
                    </tr>
<?php            
                     } 
...
if(isset($_POST['modifier'])) {
    ...
    if($id) {
        ...
        echo '<div id="confirmation_v" class="popup_v">
            <form id="confirmation-form" method="post" action="conge_depar.php">
...
if(isset($_POST['modifierpns'])) {
    ...
    if($validation != null){
        $sql2 = "UPDATE demande_conge SET  val_departement= ? where id_demande= ?";
